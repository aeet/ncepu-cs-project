...
func (tq *TutorQuery) Clone() *TutorQuery {
	...
	return &TutorQuery{
		...
		withClass:   tq.withClass.Clone(),
		withStudent: tq.withStudent.Clone(),
...
func (tq *TutorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tutor, error) {
	...
	if query := tq.withClass; query != nil {
		if err := tq.loadClass(ctx, query, nodes, nil,
			func(n *Tutor, e *Class) { n.Edges.Class = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withStudent; query != nil {
		if err := tq.loadStudent(ctx, query, nodes, nil,
			func(n *Tutor, e *Student) { n.Edges.Student = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TutorQuery) loadClass(ctx context.Context, query *ClassQuery, nodes []*Tutor, init func(*Tutor), assign func(*Tutor, *Class)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tutor)
	for i := range nodes {
	...
	query.Where(predicate.Class(func(s *sql.Selector) {
		s.Where(sql.InValues(tutor.ClassColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.tutor_class
		if fk == nil {
			return fmt.Errorf(`foreign-key "tutor_class" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tutor_class" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (tq *TutorQuery) loadStudent(ctx context.Context, query *StudentQuery, nodes []*Tutor, init func(*Tutor), assign func(*Tutor, *Student)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tutor)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Student(func(s *sql.Selector) {
		s.Where(sql.InValues(tutor.StudentColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.tutor_student
		if fk == nil {
			return fmt.Errorf(`foreign-key "tutor_student" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tutor_student" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (tq *TutorQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(tutor.Table, tutor.Columns, sqlgraph.NewFieldSpec(tutor.FieldID, field.TypeInt))
	_spec.From = tq.sql
