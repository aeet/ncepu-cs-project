...
type ResourceQuery struct {
	...
	withRole          *RoleQuery
	withUser          *UserQuery
	withAuthorization *AuthorizationQuery
...
func (rq *ResourceQuery) QueryAuthorization() *AuthorizationQuery {
	query := (&AuthorizationClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *ResourceQuery) Clone() *ResourceQuery {
	...
	return &ResourceQuery{
		...
		withRole:          rq.withRole.Clone(),
		withUser:          rq.withUser.Clone(),
		withAuthorization: rq.withAuthorization.Clone(),
...
// WithAuthorization tells the query-builder to eager-load the nodes that are connected to
// the "authorization" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *ResourceQuery) WithAuthorization(opts ...func(*AuthorizationQuery)) *ResourceQuery {
	query := (&AuthorizationClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withAuthorization = query
	return rq
}
...
func (rq *ResourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Resource, error) {
	...
	if query := rq.withRole; query != nil {
		if err := rq.loadRole(ctx, query, nodes,
			func(n *Resource) { n.Edges.Role = []*Role{} },
			func(n *Resource, e *Role) { n.Edges.Role = append(n.Edges.Role, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withUser; query != nil {
		if err := rq.loadUser(ctx, query, nodes,
			func(n *Resource) { n.Edges.User = []*User{} },
			func(n *Resource, e *User) { n.Edges.User = append(n.Edges.User, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withAuthorization; query != nil {
		if err := rq.loadAuthorization(ctx, query, nodes,
			func(n *Resource) { n.Edges.Authorization = []*Authorization{} },
			func(n *Resource, e *Authorization) { n.Edges.Authorization = append(n.Edges.Authorization, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ResourceQuery) loadRole(ctx context.Context, query *RoleQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Role)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(resource.RoleTable)
		s.Join(joinT).On(s.C(role.FieldID), joinT.C(resource.RolePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(resource.RolePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(resource.RolePrimaryKey[1]))
...
func (rq *ResourceQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Resource)
	nids := make(map[int]map[*Resource]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(resource.UserTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(resource.UserPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(resource.UserPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(resource.UserPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (rq *ResourceQuery) loadAuthorization(ctx context.Context, query *AuthorizationQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Authorization)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Resource)
	nids := make(map[int]map[*Resource]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(resource.AuthorizationTable)
		s.Join(joinT).On(s.C(authorization.FieldID), joinT.C(resource.AuthorizationPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(resource.AuthorizationPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(resource.AuthorizationPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*Authorization](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "authorization" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
