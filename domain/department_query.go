...
func (dq *DepartmentQuery) QueryMajor() *MajorQuery {
	query := (&MajorClient{config: dq.config}).Query()
...
func (dq *DepartmentQuery) QueryClass() *ClassQuery {
	query := (&ClassClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DepartmentQuery) QueryStudent() *StudentQuery {
	query := (&StudentClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DepartmentQuery) Clone() *DepartmentQuery {
	...
	return &DepartmentQuery{
		...
		withMajor:   dq.withMajor.Clone(),
		withClass:   dq.withClass.Clone(),
		withStudent: dq.withStudent.Clone(),
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := dq.withMajor; query != nil {
		if err := dq.loadMajor(ctx, query, nodes,
			func(n *Department) { n.Edges.Major = []*Major{} },
			func(n *Department, e *Major) { n.Edges.Major = append(n.Edges.Major, e) }); err != nil {
			return nil, err
		}
	}
	if query := dq.withClass; query != nil {
		if err := dq.loadClass(ctx, query, nodes,
			func(n *Department) { n.Edges.Class = []*Class{} },
			func(n *Department, e *Class) { n.Edges.Class = append(n.Edges.Class, e) }); err != nil {
			return nil, err
		}
	}
	if query := dq.withStudent; query != nil {
		if err := dq.loadStudent(ctx, query, nodes,
			func(n *Department) { n.Edges.Student = []*Student{} },
			func(n *Department, e *Student) { n.Edges.Student = append(n.Edges.Student, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DepartmentQuery) loadMajor(ctx context.Context, query *MajorQuery, nodes []*Department, init func(*Department), assign func(*Department, *Major)) error {
	...
	query.Where(predicate.Major(func(s *sql.Selector) {
		s.Where(sql.InValues(department.MajorColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.major_department
		if fk == nil {
			return fmt.Errorf(`foreign-key "major_department" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "major_department" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (dq *DepartmentQuery) loadClass(ctx context.Context, query *ClassQuery, nodes []*Department, init func(*Department), assign func(*Department, *Class)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Class(func(s *sql.Selector) {
		s.Where(sql.InValues(department.ClassColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.class_department
		if fk == nil {
			return fmt.Errorf(`foreign-key "class_department" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "class_department" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (dq *DepartmentQuery) loadStudent(ctx context.Context, query *StudentQuery, nodes []*Department, init func(*Department), assign func(*Department, *Student)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Student(func(s *sql.Selector) {
		s.Where(sql.InValues(department.StudentColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.student_department
		if fk == nil {
			return fmt.Errorf(`foreign-key "student_department" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "student_department" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
