...
func (uq *UserQuery) QueryResource() *ResourceQuery {
	query := (&ResourceClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withResource; query != nil {
		if err := uq.loadResource(ctx, query, nodes,
			func(n *User) { n.Edges.Resource = []*Resource{} },
			func(n *User, e *Resource) { n.Edges.Resource = append(n.Edges.Resource, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadResource(ctx context.Context, query *ResourceQuery, nodes []*User, init func(*User), assign func(*User, *Resource)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*User)
	nids := make(map[int]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.ResourceTable)
		s.Join(joinT).On(s.C(resource.FieldID), joinT.C(user.ResourcePrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.ResourcePrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.ResourcePrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
